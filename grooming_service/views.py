...
def home(request):
    home_services = ((Service
                     .objects.exclude(short_description__exact=''))
                     .order_by('-id'))
...
def book_appointment(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                check_appointment = ((Appointment
                                     .objects)
                                     .get(start_date_time=start_date_time,
                                     status=1))
                messages.error(request,
...
def __handle_form_case(request, appointmentForm,
                       petForm, editPetForm, form_type):
    match form_type:
        case 'edit_appointment_form':
            if appointmentForm.is_valid():
                __handle_edit_appointment_form(request, appointmentForm)
            else:
                __handle_form_errors(request,
                                     appointmentForm,
...
def __handle_edit_appointment_form(request, appointmentForm):
    ...
    try:
        ...
        request_date_time_obj = datetime.strptime(start_date_time,
                                                  '%Y-%m-%d %H:%M')
        formatted_date_time = request_date_time_obj.strftime('%Y-%m-%d %H:%M')
        if (appointment.start_date_time.strftime('%Y-%m-%d %H:%M')
                != formatted_date_time):
            if Appointment.objects.filter(start_date_time=formatted_date_time,
                                          status=1).exists():
                messages.error(request,
                               "The selected appointment slot is"
                               " no longer available",
                ...
                appointment.save()
                messages.success(request, "Your appointment has been"
...
def __handle_pet_edit_form(request, editPetForm):
    ...
    try:
        ...
        pet.save()
        messages.success(request, f"{pet.name} has been successfully updated."
                                  " Redirecting you to profile page...",
...
def __handle_pet_add_form(request, petForm):
    ...
    pet.image = image
    pet.save()
    messages.success(request, f"{pet.name} has been successfully created."
                              " Redirecting you to profile page...",
...
def cancel_appointment(request, cancel_appointment_id):
    try:
        ...
        if appointment.user == request.user:
            appointment.delete()
            appointment_time = (
                appointment.start_date_time.strftime('%Y-%m-%d %H:%M'))
            messages.success(request,
                             f"Your appointment on the {appointment_time} has"
                             " been cancelled."
                             " Redirecting you to profile page...",
